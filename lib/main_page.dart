...
class _MyAppState extends State<MyApp> with AutomaticKeepAliveClientMixin {
  ...
  submit() {
    setState(() {
      if (horizontalCounter % 4 == 0 &&
        ...
        print("at_place:$at_place");
        if (at_place == 4) {
          print("Maray dos at_place is $at_place");
          addHighScore(horizontalCounter);
        }
        print("I am executed");
  ...
  addHighScore(a) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setDouble("Latest Score", 1 ~/ a * 100);
  }

  Future<void> _loadLatestScore() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    double? score = prefs.getDouble("Latest Score");
    setState(() {
      latestScore = score;
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Master Mind Game"),
      ),
      body: Column(
